...
```
After running, the raw data of the proteins is most likely stored in `predictions-db`; the data I will extract/copy is `summary_confidences.parquet` and `summary_pairs.parquet` which is in `workspaces/data`.
...
```
Basic analyzation of data will be in: 
